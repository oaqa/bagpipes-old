...
object yamlTest {;import org.scalaide.worksheet.runtime.library.WorksheetSupport._; def main(args: Array[String])=$execute{;$skip(750); 
 
  val i = 0

  case class Foo(bar: JSONObject)

  case class RoomAnnotator(foo: String, map: Map[String, Any]);System.out.println("""i  : Int = """ + $show(i ));$skip(150); 

  val ex0 =
"""
bar:
  baz: aaa
  cas: blah""";System.out.println("""ex0  : String = """ + $show(ex0 ));$skip(277); 
...
collection-reader:
  ...
  InputDirectory: data/
""";System.out.println("""ex2  : String = """ + $show(ex2 ));$skip(42); 
  ...
  val map = JSONObject(yaml.load(ex0).asInstanceOf[java.util.Map[String, Any]]);System.out.println("""map  : scala.util.parsing.json.JSONObject = """ + $show(map ));$skip(81); 
  val map2 = JSONObject(yaml.load(ex1).asInstanceOf[java.util.Map[String, Any]]);System.out.println("""map2  : scala.util.parsing.json.JSONObject = """ + $show(map2 ));$skip(104); val res$0 = 
  ...
  map2.toString();System.out.println("""res1: String = """ + $show(res$1));$skip(43); 
  val parsedEx1 = decompose(map2) \\ "obj";System.out.println("""parsedEx1  : net.liftweb.json.JsonAST.JValue = """ + $show(parsedEx1 ));$skip(263); 
